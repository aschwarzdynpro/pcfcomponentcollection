...
<#
    Builds the MaterialBoxList PCF and packs it into Dataverse-importable
    solution zips (unmanaged + managed).

    Output (canonical "latest" — overwritten each build):
        bin\MaterialBoxList.zip          (Unmanaged)
        bin\MaterialBoxList_managed.zip  (Managed)
    ...
    src\Other\Solution.xml), so old builds stay available for roll-back:
        bin\MaterialBoxList_<version>.zip
        bin\MaterialBoxList_managed_<version>.zip
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../MaterialBoxList (wrapper)
$pcfRoot      = Join-Path $featureRoot 'MaterialBoxList'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\sst_MaterialBox.MaterialBoxList'
...
$unmanaged = Join-Path $binDir 'MaterialBoxList.zip'
$managed   = Join-Path $binDir 'MaterialBoxList_managed.zip'
...
$unmanagedV = Join-Path $binDir "MaterialBoxList_$version.zip"
$managedV   = Join-Path $binDir "MaterialBoxList_managed_$version.zip"
