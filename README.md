...
| [FuzzyLookupControl](./FuzzyLookupControl/FuzzyLookupControl/) | field | Drop-in replacement for the standard `Lookup.Simple` control. Dataverse Search (Lucene-fuzzy) typeahead across up to four columns, each hit rendered as a card (title + stacked subtitles) with highlighting. OData `additionalFilter` with `{record.…}` / `{user.…}` runtime tokens, in-place Quick-Create, swipe-to-favourite and long-press preview on touch (optionally driven by a Quick View Form), plus architecture hooks for favorites and recently used. Trilingual UI (DE / EN / FR). |
