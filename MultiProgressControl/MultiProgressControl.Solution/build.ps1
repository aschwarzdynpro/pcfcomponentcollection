...
<#
    Builds the MultiProgressControl PCF and packs it into Dataverse-importable
    solution zips (unmanaged + managed).

    Output:
        bin\MultiProgressControl.zip          (Unmanaged)
        bin\MultiProgressControl_managed.zip  (Managed)
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../MultiProgressControl (wrapper)
$pcfRoot      = Join-Path $featureRoot 'MultiProgressControl'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\wal_Progress.MultiProgressControl'
...
$unmanaged = Join-Path $binDir 'MultiProgressControl.zip'
$managed   = Join-Path $binDir 'MultiProgressControl_managed.zip'
