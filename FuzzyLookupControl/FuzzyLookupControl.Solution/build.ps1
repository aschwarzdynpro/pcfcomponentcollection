...
<#
    Builds the FuzzyLookupControl PCF and packs it into Dataverse-importable
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../FuzzyLookupControl (wrapper)
$pcfRoot      = Join-Path $featureRoot 'FuzzyLookupControl'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\wal_Lookup.FuzzyLookupControl'
...
$unmanaged = Join-Path $binDir 'FuzzyLookupControl.zip'
$managed   = Join-Path $binDir 'FuzzyLookupControl_managed.zip'
