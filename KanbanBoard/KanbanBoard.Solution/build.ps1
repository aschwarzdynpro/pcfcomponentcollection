...
<#
    Builds the KanbanBoard PCF and packs it into Dataverse-importable
    solution zips (unmanaged + managed).

    Output:
        bin\KanbanBoard.zip          (Unmanaged)
        bin\KanbanBoard_managed.zip  (Managed)
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../KanbanBoard (wrapper)
$pcfRoot      = Join-Path $featureRoot 'KanbanBoard'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\wal_Kanban.KanbanBoard'
...
$unmanaged = Join-Path $binDir 'KanbanBoard.zip'
$managed   = Join-Path $binDir 'KanbanBoard_managed.zip'
