...
<#
    Builds the ChoicePickerControl PCF and packs it into Dataverse-importable
    solution zips (unmanaged + managed).

    Output:
        bin\ChoicePickerControl.zip          (Unmanaged)
        bin\ChoicePickerControl_managed.zip  (Managed)
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../ChoicePickerControl (wrapper)
$pcfRoot      = Join-Path $featureRoot 'ChoicePickerControl'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\wal_ChoicePicker.ChoicePickerControl'
...
$unmanaged = Join-Path $binDir 'ChoicePickerControl.zip'
$managed   = Join-Path $binDir 'ChoicePickerControl_managed.zip'
