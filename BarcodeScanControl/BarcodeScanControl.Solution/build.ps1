...
<#
    Builds the BarcodeScanControl PCF and packs it into Dataverse-importable
    solution zips (unmanaged + managed).

    Output:
        bin\BarcodeScanControl.zip          (Unmanaged)
        bin\BarcodeScanControl_managed.zip  (Managed)
...
$featureRoot  = Split-Path $solutionRoot -Parent  # ../BarcodeScanControl (wrapper)
$pcfRoot      = Join-Path $featureRoot 'BarcodeScanControl'
$controlOut   = Join-Path $pcfRoot 'out\controls'
$stageRoot    = Join-Path $solutionRoot 'src\Controls\wal_BarcodeScan.BarcodeScanControl'
...
$unmanaged = Join-Path $binDir 'BarcodeScanControl.zip'
$managed   = Join-Path $binDir 'BarcodeScanControl_managed.zip'
